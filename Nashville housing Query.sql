...
-- joining both table total males and females
...
select a.* from
(select district,state,literacy,rank() over(partition by state order by literacy desc) rkk from dataset1) a
where a.rkk in (1,2,3) order by state
